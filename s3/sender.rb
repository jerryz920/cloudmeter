...
def s3_upload key
  ...
  #
  `tail -n 1 /proc/ec2_netstat >> tmpsenderstat; ifconfig eth0 | grep 'RX bytes' >> tmpsenderstat; aws s3 put-object --key #{key} --bucket sonarcloud --body tmpsenderstat`
  sleep 60
end

def start_workload (host,port,size)
  `/home/ubuntu/monitor/udp/udp_sender #{host} #{port} #{size}`
end
